...
class MegaMillions:
    ...
    def check_win(ticket: TicketNumbers, winning_numbers: TicketNumbers) -> int:
        ...
        # todo: add Jackpot based on current actual jackpot amount
    ...
    def get_match_description(ticket: TicketNumbers, winning_numbers: TicketNumbers) -> str:
        ...
        if white_matches == 0 and mega_matches:
            return "Mega Ball only"
        elif mega_matches:
            return f"{white_matches} number{'s' if white_matches != 1 else ''} + Mega Ball"
        else:
            return f"{white_matches} number{'s' if white_matches != 1 else ''}"
...
def print_summary(num_tickets: int, total_spent: float, total_won: int) -> None:
    ...
    """
    print("\nResults Summary:")
    print(f"Total tickets purchased: {num_tickets}")
    print(f"Total spent: ${total_spent:.2f}")
    print(f"Total won: ${total_won:,}")
    print(f"Net profit/loss: ${(total_won - total_spent):,.2f}")
    print(f"Return on spend: {(total_won / total_spent * 100):.2f}%")
...
def _print_probability_row(match_type: str, data: WinData, num_tickets: int) -> None:
    ...
    # Calculate standard deviation (σ) using sqrt(n*p*(1-p))
    std_dev = (num_tickets * data.odds * (1 - data.odds)) ** 0.5

    # Calculate difference as percentage of expected
    if expected > 0:
        diff_percent = ((actual - expected) / expected) * 100
        # Check if within 2 standard deviations (95% confidence interval)
        within_2sigma = abs(actual - expected) <= 2 * std_dev

        print(
            f"{match_type:<15} {expected:>9.1f} {actual:>9} {diff_percent:>9.1f}% {'Yes' if within_2sigma else 'No':>9}")
...
def main() -> None:
    ...
    """
    try:
        random.seed(os.urandom(8))  # Initialize with system entropy
        ...
        # Generate winning numbers for this game
        ...
        # Buy and check multiple tickets
